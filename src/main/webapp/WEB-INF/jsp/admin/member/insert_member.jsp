              ...
              <!-- /.card-header -->
                ...
                <div class="card-body">
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="PASSWORD_HINT" id="PASSWORD_HINT" placeholder="암호힌트를 입력해주세요." required>
                  </div>
                  <div class="form-group">
                    <label for="PASSWORD_CNSR">PASSWORD_CNSR</label>
                    <input type="text" class="form-control" name="PASSWORD_CNSR" id="PASSWORD_CNSR" placeholder="암호힌트의 답을 입력해주세요." required>
                  </div>
                  <div class="form-group">
                  ...
                  <div class="form-group">
                   ...
                   <input type="text" class="form-control" name="HOUSE_ADRES" id="HOUSE_ADRES" placeholder="집 주소를 입력해주세요." required>
                  </div>
                  <div class="form-group">
                   <label for="EMAIL_ADRES">EMAIL_ADRES</label>
                   <input type="email" class="form-control" name="EMAIL_ADRES" id="EMAIL_ADRES" placeholder="이메일을 입력해주세요." required>
                  </div>          
                  <div class="form-group">
                   <label for="ORGNZT_ID">ORGNZT_ID</label>
                   <input type="text" class="form-control" name="ORGNZT_ID" id="ORGNZT_ID" placeholder="소속기관을 입력해주세요." required>
                  </div>
                 ...
                 <c:forEach items="${codeMap}" var="sub2">
                  ...
                  <div class="form-group">
                   ...
                   <input type="text" class="form-control" name="ESNTL_ID" id="ESNTL_ID" placeholder="게시판관리 고유ID를 입력해주세요." required readonly>
                  </div>
                </div>
                <!-- /.card-body -->
            </div>
...
$(document).ready(function(){
	//emplyr_id 중복체크 이후 submit 버튼을 disabled를 false로 활성화 시키면 전송이 가능 Ajax
	//blur조건 focus의 반대말
	$("#EMPLYR_ID").bind("blur",function(){
		var emplyr_id = $(this).val();
		$.ajax({
			url:"<c:url value='/' />idcheck?emplyr_id="+emplyr_id, //@Response사용하는 클래스의 메서드 매핑 URL값. 반환값이 페이지x, text임.
			type:"get", //jsp에서 컨트롤러로 보내는 방식
			dataType:"text", //ajax결과를 컨트롤러에서 받는 방식
			success:function(result){
				if(result=="0"){ //중복id가 없으면
					alert("사용가능한 ID입니다.");
					$("#btn_insert").attr("disabled",false); //submit 활성화
				}else{ //중복id가 있으면
					alert("중복ID가 존재합니다.");
					$("#btn_insert").attr("disabled",true); //submit 비활성화
				}
			},
			error:function(){
